...
def generate_negative_examples(incomplete_graph_file, positive_examples_file, neg_examples_per_positive, output):
    ...
    n_examples = int(neg_examples_per_positive)
    ...
    for fact in positive_examples:
        ...
        number_of_negative_examples_for_this_fact = min(n_examples, len(candidate_replacement_constants))
        if (number_of_negative_examples_for_this_fact < n_examples):
            print("Warning: less than {} head examples generated for fact {} {} {}".format(n_examples,head,body,tail))
    ...
    for fact in positive_examples:
        ...
        if body != "<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>":
            ...
            number_of_negative_examples_for_this_fact = min(n_examples, len(candidate_replacement_relations))
            if (number_of_negative_examples_for_this_fact < n_examples):
                print("Warning: less than {} body examples generated for fact {} {} {}".format(n_examples,head,body,tail))
    ...
    for fact in positive_examples:
        ...
        if body == "<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>":
            ...
            number_of_negative_examples_for_this_fact = min(n_examples, len(candidate_replacement_classes)) 
            if (number_of_negative_examples_for_this_fact < n_examples):
                print("Warning: less than {} tail examples generated for fact {} {} {}".format(n_examples,head,body,tail))
            #  Sample the corresponding number of constants from the list
    ...
    for fact in positive_examples:
        (ent1, ent2, ent3) = fact
        output_file_all.write(ent1 + '\t' + ent2 + '\t' + ent3 + '\t' + "1" + '\n')
    
    output_file_heads = open(parent_folder + "/heads" + fext, "w") 
    output_file_tails = open(parent_folder + "/tails" + fext, "w") 
    negative_examples_heads = list(negative_examples_heads)
    negative_examples_tails = list(negative_examples_tails)
    num_added_heads = 0
    num_added_tails = 0
    for i in range(0,len(positive_examples)):
        coin = random.randint(1,2)
        if coin == 1:
            (ent1, ent2, ent3) = negative_examples_heads[num_added_heads] 
            num_added_heads += 1 
            output_file_heads.write(ent1 + '\t' + ent2 + '\t' + ent3 + '\t' + "0" + '\n')
            output_file_all.write(ent1 + '\t' + ent2 + '\t' + ent3 + '\t' + "0" + '\n')
        else: 
            (ent1, ent2, ent3) = negative_examples_tails[num_added_tails] 
            num_added_tails += 1 
            output_file_tails.write(ent1 + '\t' + ent2 + '\t' + ent3 + '\t' + "0" + '\n')
            output_file_all.write(ent1 + '\t' + ent2 + '\t' + ent3 + '\t' + "0" + '\n')
    output_file_tails.close()
    output_file_heads.close()
    output_file_all.close()
    print("Added {} head corruptions and {} tail corruptions.".format(num_added_heads, num_added_tails))
